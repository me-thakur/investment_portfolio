...
#Allows multiple sub-commands under one CLI.
@click.group()
def cli():
    pass
...
def show_coin_price(coin_id, currency):
    ...
    #Prints the price formatted to 2 decimal places
...
@click.command()
@click.option("--export_file", default="investments_export")
def export_investment(export_file):
    ...
    with open(export_file, 'w', newline="") as f:
        ...
        print(f"File exported successfully: {export_file}")


#Adds show_coin_price as a sub-command of cli    
cli.add_command(show_coin_price)
cli.add_command(add_investment)
cli.add_command(get_investment_value)
cli.add_command(import_investment)
cli.add_command(export_investment)
